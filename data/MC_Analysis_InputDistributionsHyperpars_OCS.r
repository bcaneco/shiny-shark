...
inputsHyperPars <- list(
  ...
  #
  cbty = list(shkln  = list(mu_shkln = -0.47969, sd_shkln = 0.04487, label = "Shark-line"),  # source: from catch vs hook depth models
              shllwR = list(mu_shll  = -4.94498, sd_shll  = 0.03262, label = "Shallow"),     # idem
              deep   = list(mu_deep  = -4.16491, sd_deep  = 0.02528, label = "Deep")),       # idem
  ...
  #
  lhk  = list(j = list(p_j = 0.3 , n_j = 12, label = "J-hook"),        # p_j = E[P(Lip|J-hook)]       ; source: best guess (p based on SJH's code)
              t = list(p_t = 0.33 , n_t = 12, label = "Tuna-hook"),    # p_t = E[P(Lip|Tuna-hook)]    ; idem
              c = list(p_c = 0.9, n_c = 12,  label = "Circle-hook")),  # p_c = E[P(Lip|Circle-hook)]  ; idem
  ...
  mrt = list(L = list(p_L = 0.1867, n_L = 6361, label = "Lip-hooked"),   # p_L = E[P(Dead|OH,lip)] ; higher for lip hooked as they didnt bite-off 
             G = list(p_G = 0.1867, n_G = 6361, label = "Gut-hooked")),  # p_G = E[P(Dead|OH,gut)]
